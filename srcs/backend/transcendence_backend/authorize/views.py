...
def login_user(request) -> JsonResponse:
    ...
    if user != None:
        ...
        access_token = create_token(jwt=jwt, 
                                    user=user, 
                                    expiration=datetime.now() + timedelta(days=expiration_time), 
                                    isa=user.last_login, second_factor=user.is_2fa_enabled)
        refresh_token = create_token(jwt=jwt,
                                     user=user,
                                     expiration=datetime.now() + timedelta(days=refresh_time),
                                     isa=user.last_login,
...
@jwt_auth_required(days=refresh_time)
def refresh_token(request, user : User) -> JsonResponse:
    ...
    jwt = JWT(settings.JWT_SECRET)
    # UserService.update_last_login(user)
    access_token = create_token(jwt=jwt, user=user, expiration=datetime.now() + timedelta(days=expiration_time), isa=user.last_login)
...
def verify_2fa(request, user : User) -> JsonResponse:
    ...
    if SecondFactorService.verify_2fa(user, code):
        jwt = JWT(settings.JWT_SECRET)
        UserService.update_last_login(user)
        access_token = create_token(jwt=jwt, user=user, expiration=datetime.now() + timedelta(days=expiration_time), isa=user.last_login, second_factor=False)
        refresh_token = create_token(jwt=jwt, user=user, expiration=datetime.now() + timedelta(days=refresh_time), isa=user.last_login, second_factor=False)
